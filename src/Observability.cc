...
bool Observability::runMatrixBuilding(const size_t mat_type, const double time_for_build,
    ...
    switch (mat_type) {
    case ORB_SLAM3::MAP_INFO_MATRIX:
        //
    ...
    case ORB_SLAM3::FRAME_INFO_MATRIX:
        //
        if (this->pFrame == NULL)
            return false;

        N = this->pFrame->mvpMapPoints.size();
        cout<< "mvp Map Points:" << N <<endl;
    ...
    case ORB_SLAM3::MAP_HYBRID_MATRIX:
        //
        if (this->mMapPoints == NULL)
            return false;
    ...
    case ORB_SLAM3::FRAME_HYBRID_MATRIX:
        //
        if (this->pFrame == NULL)
            return false;
    ...
    case ORB_SLAM3::FRAME_STRIP_OBS_MATRIX:
        // TODO
        break;
...
int Observability::runBaselineMapMatching(Frame *pFrame,
    ...
    //
    if (base_mtd == ORB_SLAM3::BASELINE_RANDOM) {
        // random shuffle
        std::random_shuffle ( vldPoints.begin(), vldPoints.end() );
    }
    else if (base_mtd == ORB_SLAM3::BASELINE_LONGLIVE) {
        // sort by life
