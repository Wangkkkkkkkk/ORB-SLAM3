    ...
    inline bool compute_H_subblock_simplied (const arma::rowvec & Xv,
        ...
        if (check_viz) {
            ...
            if ( hrl(2,0) < 0.0 + this->mBoundDepth )
                return false;
            if(u < ORB_SLAM3::Frame::mnMinX - this->mBoundXInFrame || u > ORB_SLAM3::Frame::mnMaxX + this->mBoundXInFrame)
                return false;
            if(v < ORB_SLAM3::Frame::mnMinY - this->mBoundYInFrame || v > ORB_SLAM3::Frame::mnMaxY + this->mBoundYInFrame)
                return false;
        }
